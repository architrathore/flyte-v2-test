...
def generate_report_html(title: str, color: str, config_data: dict[str, Any], 
                        success_message: str, success_details: list[str] | None = None,
...
class TrainConfig(BaseModel):
    ...
    def validate_model_configs(self):
        ...
        if not any([self.gbm_config, self.dnn_config]):
            raise ValueError("At least one of gbm_config or dnn_config should be provided")
        ...
        if self.gbm_config and self.dnn_config and not self.ensemble_config:
            raise ValueError("Both gbm_config and dnn_config are provided, ensemble_config should also be provided")
        ...
        if (not self.gbm_config or not self.dnn_config) and self.ensemble_config:
            raise ValueError("Only single base model config is provided, ensemble_config should not be provided")
        ...
        if self.ensemble_config:
            ...
            if abs(sum(weights.values()) - 1.0) > 1e-5:
                raise ValueError(f"model_weights in ensemble_config should sum to 1, got {weights}")
...
async def prepare_data(
    ...
    metrics = [
        {"title": "Training Set", "value": "10,000", "color": "#007bff", "subtitle": "samples"},
        {"title": "Test Set", "value": "2,500", "color": "#28a745", "subtitle": "samples"},
        {"title": "Features", "value": "150", "color": "#ffc107", "subtitle": "dimensions"},
        {"title": "Positive Rate", "value": "5.2%", "color": "#dc3545", "subtitle": "fraud cases"}
    ]
...
async def create_ensemble_model(
    ...
    html = generate_report_html(
        ...
        success_details=[f"<strong>Ensemble Model SHA:</strong> <code>{ensemble_sha}</code>"]
    )
...
async def generate_reports(
    ...
    """
    logger.info(f"Generating reports for experiment {experiment_id}, scores_path {scores_path}")
    ...
    metrics = [
        {"title": "Test AUC", "value": "0.867", "color": "#007bff", "subtitle": "ROC AUC"},
        {"title": "Precision", "value": "0.823", "color": "#28a745", "subtitle": "at 0.5 threshold"},
        {"title": "Recall", "value": "0.756", "color": "#ffc107", "subtitle": "at 0.5 threshold"},
        {"title": "F1 Score", "value": "0.788", "color": "#dc3545", "subtitle": "harmonic mean"}
    ]
...
async def backtest(
    ...
    experiment_artifacts = await asyncio.gather(*[
        run_single_experiment(experiment_id=exp_id, snapshot_date=snapshot_date) for exp_id in experiment_id_list
    ])
    ...
    logger.info(f"Backtest workflow returning {len(all_reports)} reports: {list(all_reports.keys())}")
    return all_reports
